...
def init(self, raster_path: Path, patch_size: int, stride: int):
...
def predict_and_visualize(model, device, ortho_path, output_path, annotations_path, batch_size, roi_poly_wgs84):
"""
...
# 1. Prepare the sliding window dataloader
...
with rasterio.open(ortho_path) as src:
    ...
    ortho_transform, ortho_crs = src.transform, src.crs
...
with torch.no_grad():
    for batch in tqdm(dataloader, desc=f"Predicting on {ortho_path.name}"):
        ...
        for i in range(preds.shape[0]):
            ...
            full_prediction_mask[y:y+patch_size, x:x+patch_size] += pred_patch
            weight_mask[y:y+patch_size, x:x+patch_size] += 1
...
# 3. Load and filter annotations for the current ortho
...
if annotations_path and annotations_path.exists():
    ...
    for idx, row in gdf.iterrows():
        if total_veg.iloc[idx] >= 40:
            veg_points.append((row.geometry.x, row.geometry.y))
        else:
            non_veg_points.append((row.geometry.x, row.geometry.y))
...
with rasterio.open(ortho_path) as src:
    ...
    img_display = np.clip((img_data - p2) / (p98 - p2), 0, 1)
    ax.imshow(np.transpose(img_display, (1, 2, 0)), extent=rasterio.plot.plotting_extent(src))
...
# --- THIS IS THE FIX: Apply ROI bounds to the plot ---
if roi_poly_wgs84:
    try:
...
# 2. Load the ROI polygon once, if provided
roi_poly = None
if args.roi_path:
    try:
...
# 3. Find all orthomosaics to process
...
# 5. Loop over each orthomosaic and process it
